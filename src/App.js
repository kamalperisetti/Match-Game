...
const getTHeRandom = () => {
  ...
  // const randomIng = imagesList[random].imageUrl

  return random
}

const initialState = {
  activeTabd: tabsList[0].tabId,
  score: 0,
  time: 60,
  imgs: imagesList[getTHeRandom()].imageUrl,
}
class MatchGame extends Component {
  ...
  tabIsClicked = event => {
    this.setState({activeTabd: event.target.id})
  }

  getFilterdImages = () => {
    const {activeTabd} = this.state
    const filteredlist = imagesList.filter(
      eachL => eachL.category === activeTabd,
    )
    return filteredlist
  }
  ...
  thumnailClicked = e => {
    ...
    this.setState({imgs: imagesList[getTHeRandom()].imageUrl})
    const image = imagesList[getTHeRandom()].id
    console.log(image)
    if (image === e.currentTarget.id) {
  ...
  renderTheGame = () => {
    const filteredThumnails = this.getFilterdImages()
    const {imgs} = this.state

    return (
      <div>
        <div>
          <img className="iii" src={imgs} alt="match" />
        </div>
        <div className="btn-container">
          <ul className="btn-list">
            {tabsList.map(eachTab => (
              <li className="list" key={eachTab.tabId}>
                <button
                  id={eachTab.tabId}
                  onClick={this.tabIsClicked}
                  className="button"
                  type="button"
                >
                  {eachTab.displayText}
        ...
        <div>
          <ul className="thumnail-contaier">
            {filteredThumnails.map(eachI => (
              <li key={eachI.id} className="list">
                <button
                  id={eachI.id}
                  onClick={this.thumnailClicked}
                  type="button"
                  className="button"
                >
                  <img
                    className="images"
                    src={eachI.thumbnailUrl}
                    alt="thumbnail"
                  />
                </button>
              </li>
